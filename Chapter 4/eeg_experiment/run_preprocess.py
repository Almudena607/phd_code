...
def run_preprocess(subjectnr,overwrite=0):
    ...
    source_behavfn = f'{datapath}\sourcedata\sub-{subjectnr}_task-fix_events.csv'
    source_filename = f'{datapath}\sourcedata\sub-{subjectnr}_task-fix_eeg.bdf'
    raw_filename = f'{datapath}\sub-{subjectnr}\eeg\sub-{subjectnr}_task-fix_eeg.bdf'
    ...
    print(f'triggers for sub {subjectnr}:', y)
    print("Unique trigger values:", np.unique(triggervalues))
    print("Counts per trigger:", {val: count for val, count in zip(*np.unique(triggervalues, return_counts=True))})

    stim_onset_sample = [x for x,y in zip(triggertimes,triggervalues) if y==40705.0] #changing 40708 to 40705.0 TODO: try 40706?, try len(y)==4800???
    ...
    if subjectnr == '22':
        ...
        stim_onset_sample = [x for x,y in zip(triggertimes,triggervalues) if y==40708] #triggers for sub 26: 9048
