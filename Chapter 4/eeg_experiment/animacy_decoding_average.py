...
#%%
def run_control_decoding(subjectnr, decode, overwrite=1):
    """
    ...
    outfn = f'{datapath}/derivatives/results/sub-{subjectnr}_{decode}.csv'
    fig1fn = f'{datapath}/derivatives/results/figures/sub-{subjectnr}_{decode}.png'
    fig2fn = f'{datapath}/derivatives/results/figures/sub-{subjectnr}_{decode}_plot.png'
    ...
    if os.path.exists(infn) and os.path.exists(behavfn):
        if os.path.exists(outfn) and not overwrite:
            ...
            for i in np.unique(T['sequencenumber']):
                ...
                testIndices = T[list(map(all,zip(T['sequencenumber']==i, T['hybrid'] == 1)))].index.values.astype(int)
...
for participant in range(0,50):
    try:
        run_control_decoding("%02i" % participant, 'animacy', 1) #participant number, control condition to decode, 0 = no overwrite
